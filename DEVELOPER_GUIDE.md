...
Runs in a background thread on viewport changes to fetch only visible features from SQLite.
- **Non-Blocking Queue**: Rather than blocking the GUI thread by calling `.wait()` on a running worker to abort it, the application uses a double-buffering queue pattern. If a worker is already running, new render parameters are stored as pending. When the active worker completes, the pending parameters are immediately picked up and a new worker is started. This prevents thread-join stutters.
- **Viewport Spatial Query**: Computes the Mercator bounding box of the current screen view. Queries the SQLite `ways` table using a bounding box filter:
...
## 7. GPS & Navigation Subsystem

The application features a modular, pluggable GPS and real-time navigation tracking architecture.

### A. Interface & Pluggability (`gps/interface.py`)
- **`GPSDevice`**: An abstract base class inheriting from `QObject`. It defines signals for `position_updated(lat, lon, speed_knots, heading, quality)` and `status_message(msg)`.
- **`GPSRegistry`**: A dynamic registration point. GPS modules register themselves (e.g. at import time), allowing the UI to discover and list all available modules.

### B. Serial Port UART Hardware GPS (`gps/serial_gps.py`)
- Reads raw ASCII sentences from serial connections at `9600` baud.
- **Background parsing**: Runs in a separate `QThread` (`SerialGPSReaderThread`) to prevent main thread blocking.
- **Custom NMEA parser**: Extracts Latitude, Longitude, Heading, Speed, and Fix Quality values from `$GNGGA` and `$GNRMC` sentences.
- **Device Discovery**: Scans standard system COM/TTY ports and includes native support for Raspberry Pi hardware UART endpoints `/dev/ttyS0` and `/dev/ttyAMA0` (utilized by uConsole).

### C. Route Traversal Simulation (`gps/mock_gps.py`)
- Simulates route progression along Web Mercator paths.
- **Configuration**: Loaded from `mock_gps_config.json`. This config allows toggling the simulation on/off (`enabled` boolean) and adjusting the travel speed (`speed_kmh` float).
- **Discovery**: Returns at most one instance of `MockGPSDevice` dynamically configured from the file.
- **Time-step updates**: Periodically (every 500ms) steps coordinates forward, interpolating positions on active route line segments.
- **Bearing Calculation**: Computes navigation heading angles relative to the route slope at each step:
  $$\text{heading} = (450 - \text{degrees}(\text{atan2}(dy, dx))) \pmod{360}$$
- Emits standard `position_updated` signals identical to a real hardware sensor.

### D. Navigation State Controller (`viewer.py`)
- Navigation operates under three states: `inactive`, `navigating`, and `paused`.
- Starts when a route is computed and the user triggers **Start Nav**.
- Automatically centers the map viewport on the active coordinate if **Auto-Center** is checked.
- Handles route clearing and cancel actions symmetrically: cancelling navigation terminates device timers/threads and resets map markers.

### E. Text-to-Speech & Sound Alerts (`gps/speech.py`)
- **`TTSManager`**: Coordinates spoken instructions and alert sounds.
- **`QTextToSpeech`**: Leverages PySide6's cross-platform speech synthesizer to read directions asynchronously. Spoken instructions are kept identical to the text directions transcript generated by the pathfinder (excluding coordinates).
- **`QSoundEffect`**: Plays a short programmatic beep (`notification.wav`) if a distance threshold is skipped (due to high travel speed) or if a previous announcement is cut off by a new instruction.
- **Threshold Snapping**: Compares vehicle progress against target distances loaded dynamically from the profile (e.g. `tts_distances` in `config.json`). Uses projection logic to estimate remaining meters to junction points.

---

## 8. Guide to Extending the App
